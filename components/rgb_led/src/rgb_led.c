            ...
            case 1: // Pulsating LEDs
                ...
                for(uint8_t i = 0; i < RGB_LED_KEYBOARD_NUMBER; i++)
                {
                    ...
                    if(rgb->v > 0)
                    {
                        ...
                        // If the value is one of the 2 first leds, set the notification leds
                        if(i > 0 && i < 2){
                            ESP_ERROR_CHECK(rgb_notif->set_pixel(rgb_notif, i, red, green, blue));
                        }
                    }
                }
